...
def backward_run(scale, weights):
    ...
    syn2 = Synapses(IN_neuron, PC_neuron, on_pre='''v_post += -w*mV''', delay=0.7 * ms, dt=p.dt * ms)
...
def forward_run(scale, weights):
    ...
    for w1 in weights:
        ...
        plotting(PC_M, IN_M, label)
        spiketrain = make_spiketrain(PC_S)
        output_PC = pd.concat([output_PC, analyze_exp(hidden_state, input_theory,0, spiketrain,samples, w, scale)], axis = 0)
        output_IN = pd.concat([output_IN, analyze_exp(hidden_state, input_theory,0, spiketrain,samples, w, scale)], axis = 0)

    return output_PC, output_IN, spiketrain


def plotting(PC_M, IN_M, label):
    ...
    fig.suptitle('State Monitors, weigth' + label)
    fig.legend()
    plt.show()
