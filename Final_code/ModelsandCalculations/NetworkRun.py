...
def backward_run(scale, weights, delay, seed):
    ...
    for w1 in weights:
        ...
        output = pd.concat([output, analyze_exp(hidden_state, input_theory,0, spiketrain,samples, scale, w, PC_S.count)], axis = 0)
...
def neutral_run(scale, weights,seed):
